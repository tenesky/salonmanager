...
class DbService {
  ...
  /// Retrieves the onboarding preferences for the currently logged in user.
  ///
  /// This method looks up a record in the `customer_preferences` table where
  /// the `user_id` matches the current authenticated user's id.  If no
  /// preferences exist (i.e. the user has never completed the onboarding)
  /// the method returns null.  Any database error other than a 404/406
  /// (no rows) will be rethrown to the caller.  Returned values include
  /// all columns from the table such as gender, language, hair colour,
  /// hair length, hair structure and the boolean `onboarding_complete`.
  static Future<Map<String, dynamic>?> getUserPreferences() async {
    ...
    final response = await _client
        ...
        .execute();
    if (response.error != null) {
      // A 404 or 406 indicates no row; return null instead of throwing.
      if (response.status == 404 || response.status == 406) {
        return null;
      }
      throw response.error!;
  ...
  /// Inserts or updates the onboarding preferences for the current user.
  ///
  /// If a row with the same `user_id` already exists in the
  /// `customer_preferences` table, the provided values will be merged into
  /// the existing record.  Otherwise a new row will be inserted.  The
  /// `onboardingComplete` flag indicates whether the user has finished all
  /// onboarding steps.  Callers should set this to `false` when saving
  /// personal details for salon owners (who must still complete the
  /// business onboarding) and to `true` for customers or after the salon
  /// owner finishes their salon setup.  The method automatically
  /// timestamps the update via the `updated_at` column.  Errors
  /// encountered during the operation are rethrown.
  static Future<void> upsertUserPreferences({
    required String gender,
    required String language,
    required String hairColor,
    required String hairLength,
    required String hairStructure,
    String? firstName,
  ...
  /// Marks the onboarding as complete for the current user.  Only the
  /// `onboarding_complete` field is updated; other preference fields are
  /// left unchanged.  If no preferences row exists for the user, a new
  /// row will be created with `onboarding_complete = true`.  Use this
  /// method after salon owners finish the second part of onboarding.
  ...
  /// Returns `true` if the current user has completed the onboarding
  /// process.  This reads the `onboarding_complete` flag from the
  /// `customer_preferences` table.  If no record exists the method
  /// returns false.  Callers should use this to decide whether to
  /// redirect the user to the onboarding flow or directly to the
  /// home page.
  static Future<bool> hasCompletedOnboarding() async {
    final Map<String, dynamic>? prefs = await getUserPreferences();
    if (prefs == null) {
      return false;
    }
    final dynamic value = prefs['onboarding_complete'];
    if (value is bool) {
      return value;
    }
    if (value is int) {
  ...
  /// Upserts the user's first name into the customer_preferences table.  This
  /// method should be called after registration to ensure the first name
  /// persists across reinstallations and devices.  It creates the row if
  /// it does not exist or updates the existing record without
