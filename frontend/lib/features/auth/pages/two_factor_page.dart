...
// Import the database service to check onboarding state and salon profile.
  ...
  Future<void> _verifyCode() async {
    ...
    try {
      ...
      if (success) {
        ...
        // Persist the user's first name in Supabase if it was stored locally.
        try {
          final prefs = await SharedPreferences.getInstance();
          final String? localFirstName = prefs.getString('profile.firstName');
          if (localFirstName != null && localFirstName.isNotEmpty) {
            await DbService.setFirstName(localFirstName);
          }
        } catch (_) {
          // ignore errors writing first name
        }
        // Determine whether the user has already completed the onboarding.
        // If onboarding is done, skip straight to the home page; otherwise
        // navigate to the relevant onboarding screen.  Salon owners may
        // need to complete two stages (personal and business).
        bool completed = false;
        Map<String, dynamic>? salon;
        try {
          completed = await DbService.hasCompletedOnboarding();
          if (_role == 'salon') {
            salon = await DbService.getSalonProfile();
          }
        } catch (_) {
          // If an error occurs, assume onboarding not completed so the user is
          // guided through the flow again.  In most cases this will only
          // happen if the preferences table does not exist or the network
          // request fails.
          completed = false;
          salon = null;
        }
        if (_role == 'salon') {
          if (!completed) {
            // The personal onboarding has not been completed yet; start there.
            ...
            // Personal onboarding done but no salon exists yet; proceed to salon onboarding.
            Navigator.of(context).pushNamedAndRemoveUntil(
              '/onboarding-salon',
              (route) => false,
            );
          } else {
            // Onboarding complete; go directly to the home page.
            Navigator.of(context).pushNamedAndRemoveUntil('/home', (route) => false);
          }
        } else {
          // Customer role: if onboarding is complete, go to home, else start onboarding.
          if (completed) {
            Navigator.of(context).pushNamedAndRemoveUntil('/home', (route) => false);
          } else {
            Navigator.of(context).pushNamedAndRemoveUntil(
