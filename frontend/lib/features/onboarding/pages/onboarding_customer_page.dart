...
// Import Supabase data service to persist onboarding details remotely.
...
class _OnboardingCustomerPageState extends State<OnboardingCustomerPage> {
  ...
  Future<void> _savePreferences() async {
    ...
    // Persist preferences to Supabase so they survive app reinstallations.
    // The `onboardingComplete` flag remains false for salon owners because
    // they still need to complete the salon onboarding.  For customers it
    // is set to true so they skip onboarding on the next login.
    try {
      // Include the first name if it exists in local storage.  This ensures
      // that the user's greeting persists across devices.  If no first
      // name is available the field will be omitted and any existing
      // value in the database remains unchanged.
      final String? firstName = prefs.getString('profile.firstName');
      await DbService.upsertUserPreferences(
        gender: _selectedGender ?? '',
        language: _selectedLanguage ?? '',
        hairColor: _selectedHairColor ?? '',
        hairLength: _selectedHairLength ?? '',
        hairStructure: _selectedHairStructure ?? '',
        firstName: (firstName != null && firstName.isNotEmpty) ? firstName : null,
        onboardingComplete: _role != 'salon',
      );
    } catch (_) {
      // Ignore backend errors here; local preferences are still saved and
      // onboarding will continue.  The user can reattempt saving from the
